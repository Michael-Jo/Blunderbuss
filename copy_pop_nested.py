...
print( f" dataNest_copy= { data_copy }")

print("-"*15, "COMPREHESION")
data = "ABC"
data1 = { i for i in data } # ini jadi data set
print( f" data1 = { data1 }, type = { type (data1) }")
data1 = { i : i*3 for i in data }
print( f"data1 = {data1}, type = { type ( data1 )}" )
print("-"*15, "2.COMPREHESION")
data1 = ["x", "y", "z"]
data2 = [ 7 , 8, 9 ]
dataFinal = { k : v**3 for k,v in zip( data1, data2 )} #Jangan terbalik atau TypeError unsupported operand type(s) for ** or pow(): 'str' and 'int'
print ( f"Data Final = { dataFinal }")

print("-"*15, "3.COMPREHESION")
